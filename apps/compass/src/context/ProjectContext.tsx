...
export function ProjectProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!selected || !autoRefreshEnabled || !registryMode) {
      setRefreshStatus((prev) => (prev.state === 'watching' ? { state: 'idle' } : prev))
      return
    }

    setRefreshStatus((prev) =>
      prev.state === 'updated' || prev.state === 'refreshing' ? prev : { state: 'watching' },
    )
    ...
    const showUpdatedBanner = (snapshotVersion: string) => {
      ...
      updatedBannerTimerRef.current = setTimeout(() => {
        setRefreshStatus((prev) =>
          prev.state === 'updated' ? { ...prev, state: 'watching' } : prev,
        )
      }, UPDATED_BANNER_MS)
    }
  ...
  }, [autoRefreshEnabled, loadProject, registryMode, selected])
