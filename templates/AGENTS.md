# AGENTS.md — project entry for coding agents

**God's Eye chain (read in parallel when substantive work starts):**

1. `.cursor/rules/gods-eye-context-intent.mdc` — **START HERE**
2. `docs/37_GODS_EYE.md` §0 — portable law (or `$GODS_EYE_ROOT/docs/37_GODS_EYE.md`)
3. `docs/GODS_EYE_REPO_OVERLAY.md` — local vocabulary (if present)
4. `docs/GODS_EYE_GRAND_SPEC.md` — router (if present)
5. This file → `docs/14_SESSION_HANDOFF.md`

**Laws:** `+#` only on memory docs · this repo only · Tier C default · memory + wire before code unless the user names UI/copy/implementation.

**After meaningful work:** append handoff **Recent sessions**; Tier 2+ also changelog + learning log.
