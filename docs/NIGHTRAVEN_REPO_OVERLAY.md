...
| **LM Studio auto-patch myth** | Review-only script: `.md` not `.json`; dry-run = skill/gap not patch stats; zsh `print -n`+`read` on macOS; see §6 zsh one-liner + expected output |
